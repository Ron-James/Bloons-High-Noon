...
public class TurretTargetTrigger : MonoBehaviour
{
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (other.tag == "Enemy")
        {
            ...
            if (other.gameObject.GetComponent<EnemyFollower>().OnFire && GetComponentInParent<TurretExpoDamage>() != null)
            {
    ...
    void RemoveDeadEnemies()
    {
        for (int loop = 0; loop < enemiesInRange.Count; loop++)
        {
            if (enemiesInRange[loop] != null)
            {
                if (enemiesInRange[loop].GetComponent<EnemyHealth>().Health <= 0)
                {
